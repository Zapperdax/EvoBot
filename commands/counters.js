...
module.exports = {
    ...
    async execute(interaction) {
        ...
        const countsWithusernames = await Promise.all(
            counts.map(async (count) => {
                try {
                    ...
                    return { ...count.toObject(), username: member.user.username };
                } catch (error) {
                    // Handle errors, such as if the member is not found
                    console.error(`Error fetching member for user with ID ${count.id}:`, error.message);
                    return { ...count.toObject(), username: 'Unknown' }; // Default value or handle as needed
                }
            })
        );
        ...
        function generateEmbed(page) {
            ...
            for (let i = startIndex; i < endIndex && i < countsWithusernames.length; i++) {
                embed.addFields({
